...
@interface MyDonwLoadViewController ()

@end

@implementation MyDonwLoadViewController
...
- (void)viewDidLoad {
    ...
    NSArray* selectNameArray = [NSArray arrayWithObjects:@"上传时间", @"推荐最多", @"分享最多", nil];
    for (int i = 0; i < 3; i++) {
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView.tag == 155) {
        ...
        [_firstCell.photoButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"headPhoto%lu.jpg",indexPath.row + 5]] forState:UIControlStateNormal];
    } else if (tableView.tag == 156) {
        _firstCell = [_secondTableView dequeueReusableCellWithIdentifier:@"photo" forIndexPath:indexPath];
        [_firstCell.photoButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"headPhoto%lu.jpg",indexPath.row + 10]] forState:UIControlStateNormal];
    } else if (tableView.tag == 157) {
        _firstCell = [_thirdTableView dequeueReusableCellWithIdentifier:@"photo" forIndexPath:indexPath];
        [_firstCell.photoButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"headPhoto%lu.jpg",indexPath.row + 15]] forState:UIControlStateNormal];
    }
